...
**Re-measurement (2026-09-09, rf_v11/if_v10) — and why "100%" needs a corpus caveat before it's cited anywhere:**

The ~49% figure above predates rf_v10 by two model generations and cannot be cited as current. Re-run against the production `rf_v11`/`if_v10` models (`logSguarDian-vulnerable-project`, `docker-compose.direct.yml` bypassing the WAF, `attack-sim/large_corpus_requests.json` — 200 cmdi payloads at `POST /admin/ping`, fresh `logsguardian-events.db`) found **100% of cmdi payloads (200/200) labeled `predicted_class=sqli`, zero labeled `cmdi`** — 198/200 (99.0%) still correctly blocked (`verdict=block`), the remaining 2 logged as `pass_anomaly`. Model identity was confirmed byte-for-byte (`sha256sum` of the container's loaded `.onnx` files against `training/models/*.onnx` on `develop`), and the result was reproduced with direct ONNX inference on 10 decoded sample payloads (bypassing Docker/HTTP/`app.js` entirely) — same 10/10 top-class-`sqli` outcome, so this is a genuine model property on this input distribution, not an app-level artifact.

**This 100% figure is corpus-specific, not a general "cmdi is 100% confused with sqli" statement, and should always be cited with that scope attached.** `attack-sim/large_corpus_requests.json`'s cmdi slice is a single synthetic template family — blind timing/string-comparison oracles (`str=$(echo X); str1=${#str}; if [ N -ne ${str1} ]; then sleep 0; else sleep 1; fi`, `$((a+b))` arithmetic, `-ne`/`-eq` comparisons, heavy quoting) — 115/200 explicitly contain `sleep`, and the other 85 share the same comparison/arithmetic/quote-heavy structure. This is exactly the failure mode already root-caused above (§5.1, sleep-based blind cmdi structurally resembling SQLi time-based blind injection via the shared `semicolon_count` feature) — the corpus used for this re-measurement happens to be **entirely** that narrow subtype, not a representative cross-section of cmdi techniques (contrast with the broader, SecLists-sourced training/val cmdi corpus).

**This reconciles against, and does not contradict, the offline `rf_v11` test-set number** (`training/models/rf_v11_metadata.json`: `per_class_f1.cmdi = 0.9461`). That F1 is computed the same way as attribution accuracy — `classification_report(y_val, best_rf.predict(X_val))`, i.e. exact `predicted_class` match against true label, not verdict=block (`training/notebooks/03_random_forest.ipynb`) — so it is a real, comparable attribution metric, just measured against a diverse offline validation set rather than this one narrow live-corpus template family. Both numbers are correct for what they measure: **94.6% cmdi attribution accuracy on the diverse offline validation set; 0% cmdi attribution accuracy (100% confusion with sqli) on this specific blind-timing-oracle-only live corpus.** Blocking/security posture is unaffected in either case (99%+ still blocked); only the class *label* used for `attacks`/`endpoints` reporting is wrong, and only for this technique family.

See `logSguarDian-vulnerable-project/attack-sim/cmdi_sqli_attribution_rfv11_2026-09-09.json` (predicted_class/verdict breakdown from `detection_events`) and `results_config2-rfv11.json` (per-payload block status) for the raw run output.
...
2. **cmdi/sqli class-attribution confusion** (§5.1) — measured at ~49% (July 2026, pre-rf_v10), re-measured at **100%** on the same narrow sleep/timing-oracle-style corpus family (2026-09-09, rf_v11/if_v10) — every cmdi payload in that 200-payload corpus labeled `sqli` in `attacks`/`endpoints` reporting, despite 99% still being correctly blocked. Corpus-specific to this blind-oracle technique family, not a general cmdi property — offline validation F1 for cmdi attribution is 0.9461 (`rf_v11_metadata.json`) on a more diverse corpus. Affects triage accuracy, not protection.
