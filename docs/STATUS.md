...
| **cmdi/sqli class-attribution confusion** | Documented, not fixed | Re-measured 2026-09-09 against rf_v11/if_v10 on the same narrow sleep/timing-oracle cmdi corpus family that produced the stale ~49% figure (July 2026, pre-rf_v10): **100%** of that corpus's cmdi payloads (200/200) get labeled `sqli` in reporting, 99% still correctly blocked. Corpus-specific to blind-oracle cmdi techniques, not a general property — offline validation F1 for cmdi attribution is 0.9461 (`training/models/rf_v11_metadata.json`) on a more diverse corpus. Affects triage accuracy in `attacks`/`endpoints` output, not detection. See `docs/config2-results-v1.md` §5.1 and `docs/limitations.md` §1. |
