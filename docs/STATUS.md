...
| Artifact | Verified |
|----------|---------|
| `packages/extractor/src/` — 75-feature TS extractor (vigente desde rf_v11/if_v10; grew from 73 via `distinct_shell_command_count` + `shell_to_path_ratio`, added for the v11 compound-cmdi fix — see `docs/feature-spec.md`) | 68/68 tests pass, including numeric parity against the Python reference |
...
| `packages/core/src/worker.ts` — ONNX inference (RF: 69 features, IF: 63) | Parity verified against `parity_report.json` |
...
| `training/models/{rf,if}.onnx` — currently rf_v11 / if_v10, worker-pool architecture | Coverage: sqli 100%, xss 100%, path_traversal 93%, cmdi 100% (all ≥80% gate); parity <0.1% |
