...
describe("attacks inspect", () => {
  ...
  test("--format json for cmdi: f1 value is 0.9516 (rf_v11 test-set read, training/results/v11_test_results.json)", () => {
    withTempDir(() => {
      runConfigInit();
      const spy = jest.spyOn(console, "log").mockImplementation(() => {});
      ...
      // Regenerated by training/evaluate_test.py (Fase 3, R2 one-time read of
      // the locked test set against rf_v11/if_v10 — the models actually
      // published in npm). Previously this asserted 0.8954, the rf_v3 figure
      // frozen in class_metrics.json since 2026-06 — two generations behind
      // the published model.
      expect(parsed.detection_rate.model).toBe("rf_v11");
      expect(parsed.detection_rate.f1).toBe(0.9516);
      expect(parsed.detection_rate.auc_roc).toBe(0.9997);
