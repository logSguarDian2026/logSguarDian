...
describe("attacks inspect", () => {
  ...
  test("--format json for cmdi: f1 value is 0.9239 (rf_v11 test-set read, training/results/v11_test_results.json)", () => {
    withTempDir(() => {
      runConfigInit();
      const spy = jest.spyOn(console, "log").mockImplementation(() => {});
      ...
      // Regenerated by training/evaluate_test.py after the test set was
      // re-cut and rf_v11/if_v10 retrained against it (a provenance gap had
      // orphaned the previous evaluation's exact model/test-set pairing —
      // see docs/limitations.md and training/results/v11_test_results.json's
      // eval_note). Previously this asserted 0.9516 from the earlier
      // (superseded) read.
      expect(parsed.detection_rate.model).toBe("rf_v11");
      expect(parsed.detection_rate.f1).toBe(0.9239);
      expect(parsed.detection_rate.auc_roc).toBe(0.999);
